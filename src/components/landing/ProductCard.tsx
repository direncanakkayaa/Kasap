...
export default function ProductCard({
  ...
  return (
    ...
    >
      {/* Card Top – Visual */}
      <div className="relative h-56 bg-charcoal-black flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-black/60 backdrop-blur-md border border-white/10">
          <span className="text-[10px] text-ivory/60 font-semibold tracking-widest uppercase">
      ...
      {/* Card Body */}
      <div className="p-6">
        <h3 className="font-display text-xl font-bold text-ivory group-hover:text-antique-gold transition-colors">
          {product.name}
        </h3>
        <p className="text-sm text-ivory/40 mt-2 line-clamp-2 leading-relaxed">
          {product.description}
        </p>
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/5">
          <div className="flex flex-col">
            <span className="text-[10px] text-ivory/30 uppercase tracking-widest">Başlayan Fiyat</span>
            <span className="text-2xl font-display font-bold text-white">₺{product.price}</span>
          </div>
          <button className="w-12 h-12 flex items-center justify-center font-bold text-deep-espresso bg-antique-gold rounded-2xl
